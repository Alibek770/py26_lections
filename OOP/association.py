# Ассоциация - означает что внутри одного обьекта будет существовать другой объект в качестве аттрибута
# 1. Композиция - сильная связь
# 2. Агрегация - слабая связь

# Композиция - это когда стена не существует отдельно от комнаты.
# Она создается при создании комнаты и полностью управляется классом комнаты.
# class Wall:
...
# Агрегация - это когда экземпляр двигателя создается где-то в другом месте и передается в класс Авто в качестве параметра
# class Engine:
#     country = 'Germany'
...
#         self.color = color
#         self.battry = battery
#         # когда мы прнимаем в качестве параметра - агрегация
